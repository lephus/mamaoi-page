...
/**
 * Placeholder identity for speakers not yet announced — a tinted role icon +
 * "coming soon" treatment instead of an empty grey avatar. Aligned to the order
 * of the pending speakers (bác sĩ → heart, dược sĩ → beaker, khách mời → star).
 */
const SPEAKER_PENDING = [
  // Bác sĩ Nhi khoa — heart (chăm sóc)
  {
    tone: "bg-info-faded text-info",
  ...
  return (
    <>
      ...
      <main>
          ...
          <Reveal className="mx-auto grid max-w-5xl items-center gap-10 rounded-3xl bg-primary-faded p-8 sm:p-12 md:grid-cols-2 md:gap-16">
            ...
            <div className="order-2 text-center md:text-left">
              ...
              <p className="mt-5 text-lg leading-7 text-ink-faded">
                Bé Gạo là người bạn nhỏ của cộng đồng Mama Ơi — cô bé sẽ cùng mẹ và bé đi
                hết một ngày thật vui: từ lúc check-in, qua từng hoạt động, đến khi ra về.
              </p>
              ...
              <div className="mt-6 inline-flex rotate-[-4deg] items-center gap-3 rounded-xl bg-white px-4 py-3 shadow-[var(--shadow-card)] ring-1 ring-primary-border">
                ...
                <span className="text-left leading-tight">
                  ...
                  <span className="block text-lg font-extrabold text-ink">Passport</span>
                </span>
              </div>

              <p className="mt-6 text-lg leading-7 text-ink-faded">
                Đến check-in, mỗi mẹ nhận một tấm{" "}
                <strong className="text-ink">MAMA ƠI PASSPORT</strong> xinh xắn. Cùng bé Gạo
                ghi dấu qua mỗi hoạt động trong ngày — và mang tấm passport về nhà làm kỷ niệm.
              </p>
        ...
        <section className="px-5 py-14 sm:py-16">
          <div className="mx-auto max-w-6xl">
            ...
            <div className="mt-12 space-y-6">
              {EVENT_SPEAKERS.filter((s) => s.photo).map((s) => (
                <Reveal key={s.name}>
            ...
            {/* Not-yet-announced speakers — designed "coming soon" cards, not empty avatars. */}
            <ul className="mt-6 grid gap-4 sm:grid-cols-3">
              {EVENT_SPEAKERS.filter((s) => !s.photo).map((s, i) => {
                const p = SPEAKER_PENDING[i % SPEAKER_PENDING.length];
                return (
                  <Reveal key={s.name} as="li" delay={(i % 3) * 60} className="h-full">
                    <div className="group flex h-full flex-col items-center rounded-2xl bg-white p-6 text-center shadow-[var(--shadow-card)] ring-1 ring-line transition-[translate,box-shadow] duration-300 ease-[cubic-bezier(0.22,1,0.36,1)] hover:-translate-y-1.5 hover:shadow-[var(--shadow-hover)]">
                      <span
                        className={`flex h-20 w-20 items-center justify-center rounded-full transition-[scale] duration-300 ease-[cubic-bezier(0.22,1,0.36,1)] group-hover:scale-110 ${p.tone}`}
                      >
                        <svg
                          viewBox="0 0 24 24"
                          fill="none"
                      ...
                      <span className="mt-2 inline-flex items-center gap-1.5 rounded-full bg-cream px-3 py-1 text-xs font-semibold text-ink-faded ring-1 ring-line">
                        <span aria-hidden="true" className="h-1.5 w-1.5 rounded-full bg-ink-placeholder" />
                        {s.role}
                      </span>
                    </div>
                  </Reveal>
              ...
              {EVENT_TIMELINE.map((item, i) => {
                ...
                return (
                  <li key={item.title} className="relative mb-6 last:mb-0">
                    <Reveal className={`relative ${left ? "reveal-left" : "reveal-right"}`}>
                      ...
                      >
                        <div className="rounded-2xl bg-white p-5 shadow-[var(--shadow-card)] transition-[translate,box-shadow] duration-300 ease-[cubic-bezier(0.22,1,0.36,1)] hover:-translate-y-1 hover:shadow-[var(--shadow-hover)]">
                          ...
                          <h3 className="mt-2.5 text-lg font-bold text-ink">{item.title}</h3>
        ...
        <section
          id="dang-ky"
          className="scroll-mt-20 px-5 py-14 sm:py-16"
        >
